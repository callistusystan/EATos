...
class foodContract : public contract {
 public:
  ...
  // @abi action
  void createfood(account_name curOwner, uint64_t qr_code, string food_name, string expiry_date, string image,uint32_t count, string units, uint32_t price) {
    require_auth(curOwner);
    // Check if curOwner has food
  ...
  private:
    ...
    struct food {
      ...
      EOSLIB_SERIALIZE(food, (owner)(prev_owners)(qr_code)(food_name)(expiry_date)(image)(count)(units)(price))
    };
    ...
    struct sale {
      ...
      EOSLIB_SERIALIZE(sale, (sale_id)(type_of_sale)(seller)(qr_code)(prev_owners)(food_name)(expiry_date)(image)(count)(units)(price)(description))
    };
